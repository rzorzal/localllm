...
fn group_variants(files: Vec<(String, u64)>) -> Vec<(String, Vec<String>, u32)> {
    ...
    for (name, size) in files {
        let Some(tag) = quant_tag_of(&name) else {
            continue;
        };
        ...
        table.insert(
            ...
            vs.iter()
                .map(|v| {
                    (
                        v.quant.to_string(),
                        v.files.iter().map(|f| f.to_string()).collect(),
                        v.size_mb,
                    )
                })
                .collect(),
...
fn write_generated(
    table: &BTreeMap<String, Vec<(String, Vec<String>, u32)>>,
) -> anyhow::Result<()> {
...
mod tests {
    ...
    fn variant_key_strips_shard_suffix() {
        assert_eq!(
            variant_key("gemma-2-27b-it-Q6_K-00001-of-00002.gguf"),
            "gemma-2-27b-it-Q6_K"
        );
    ...
    fn quant_tag_rejects_suffixed_variants() {
        ...
        assert_eq!(
            quant_tag_of("gemma-2-27b-it-Q6_K-00001-of-00002.gguf"),
            Some("Q6_K")
        );
    }
    ...
    fn group_variants_sums_split_sizes_and_sorts_files() {
        ...
        assert_eq!(
            ...
            vec![
                (
                    "Q4_K_M".to_string(),
                    vec!["m-q4_k_m.gguf".to_string()],
                    2u32
                ),
                (
                    "Q6_K".to_string(),
                    vec![
                        "m-Q6_K-00001-of-00002.gguf".to_string(),
                        "m-Q6_K-00002-of-00002.gguf".to_string()
                    ],
                    5u32
                ),
            ]
